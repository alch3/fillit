...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/02/22 13:23:58 by rcavadas          #+#    #+#             */
/*   Updated: 2016/02/22 13:24:00 by rcavadas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		**ft_sqr_filler(int *tab, char **sqr, int *size)
{
	...
	while (tab[letter] != -1)
	{
		...
		ft_putchar((char)letter + 'A');ft_putchar('\n');
		ft_putstr("Value of X :");ft_putnbr(X);ft_putchar('\n');
		ft_putstr("Value of Y :");ft_putnbr(Y);ft_putchar('\n');

		if ((can_i_write(tab[letter], sqr, *size, coord)) == 0 && (letter <= 0))
		{
			ft_putendl("INCREMENTATION SQR");
			sqr = ft_sqrinc(sqr, size);
			X = g_coordsav[letter][0];
		...
		ft_putstr("Value of g_coord x :");ft_putnbr(g_coordsav[0][0]);ft_putchar('\n');
		ft_putstr("Value of g_coord y :");ft_putnbr(g_coordsav[0][1]);ft_putchar('\n');
		X = 0;
		Y = 0;
	sleep(1);
	}
